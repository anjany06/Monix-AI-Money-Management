...
// 4. Weekly Spending Digest
export const sendWeeklyDigest = inngest.createFunction(
  {
    id: "send-weekly-digest",
    name: "Send Weekly Spending Digest",
  ...
  async ({ step }) => {
    ...
    for (const user of users) {
      await step.run(`send-digest-${user.id}`, async () => {
        ...
        const avgDailySpend = totalSpent / expenseTransactions.length;
        ...
        await sendEmail({
          to: user.email,
          subject: `Your Weekly Spending Digest`,
          react: EmailTemplate({
            ...
            data: {
              ...
              netChange: (totalEarned - totalSpent).toFixed(2),
              transactionCount: transactions.length,
